...
> **Current development version:** V0.4.2 (build 9). This hardening build improves background network refresh behavior, release validation, and distribution consistency while keeping the Live Activity and Shortcuts behavior from V0.4.1. The committed AltStore source is updated by the tagged-release workflow, so distributed versions may intentionally lag behind `main` until a release tag is published.
